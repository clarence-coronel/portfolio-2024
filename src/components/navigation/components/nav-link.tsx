  ...
  ({ path, title }, ref) => {
    return (
      ...
      <Link
        ...
        href={path}
      >
        <span className="pointer-events-none md:group-hover:-translate-y-[100%] md:group-hover:opacity-0 opacity-100 block  duration-300">{`<${title}>`}</span>
        <span className="pointer-events-none md:group-hover:-translate-y-[100%] md:group-hover:opacity-100 opacity-0 block translate-y-[100%]  duration-300">{`<${title}/>`}</span>
      </Link>
    );
  }
);
